...
func TestFilterToString(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{Regexp("field1", "abc").Regexp("field2", "def"), `(field1 eq abc) (field2 eq def)`},
		{Regexp("field1", "abc").NotEqualInt("field2", 17), `(field1 eq abc) (field2 ne 17)`},
...
func TestFilterMatch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{f: Regexp("s", "abc").EqualBool("b", true), o: &S{}},
		...
		{f: Regexp("s", "abc").EqualBool("b", true), o: &S{S: "abc"}},
		{f: Regexp("s", "abcd").EqualBool("b", true), o: &S{S: "abc"}},
		{f: Regexp("s", "abc").EqualBool("b", true), o: &S{S: "abc", B: true}, want: true},
